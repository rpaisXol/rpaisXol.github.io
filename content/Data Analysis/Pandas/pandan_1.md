...
> [!abstract] 개요
> * **개요:** Python에서 표 형태의 데이터를 다루고 분석하기 위한 라이브러리다.
> * **주요 기능:** 데이터 입출력, 탐색, 정제, 변환, 결합, 집계, 분석
> * **주요 활용:** Excel, CSV 등의 데이터를 불러와 데이터 전처리와 분석

판다스에서 제공하는 데이터 타입은 2가지이다.
1) Series = 인덱스가 있는 1차원 배열 데이터
2) DataFrame = Series가 여러 개 모인 2차원 배열(표 형태) 데이터
...
### 1) 시리즈의 인덱스
- 파이썬 언어의 인덱스의 개념은 위치 개념이지만, 판다스의 데이터타입의 인덱스 개념와 레이블 인덱스를 구분 
- 판다스에서 인덱스라 함은 레이블 인덱스를 의미
- 레이블 인덱스를 지정하지 않으면 자동으로 0, 1, 2, ....가 부여
